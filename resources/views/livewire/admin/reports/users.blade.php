                ...
                @foreach ($reports as $r)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40" wire:key="reports-{{ $r->id }}">
                        ...
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $r->reported->username) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($r->reported->avatar) }}" alt="{{ $r->reported->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="font-medium text-xs">{{ $r->reported->username }}</p>
                                    <p class="text-[11px] leading-3 text-gray-600 pt-2">{{ $r->reported->email }}</p>
                                </div>
                            </a>
                        </td>
                        ...
                        {{-- Reporter --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $r->reporter->username) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($r->reporter->avatar) }}" alt="{{ $r->reporter->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="font-medium text-xs">{{ $r->reporter->username }}</p>
                                    <p class="text-[11px] leading-3 text-gray-600 pt-2">{{ $r->reporter->email }}</p>
                                </div>
                            </a>
                        </td>
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div id="table-options-dropdown-{{ $r->id }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">
                                        ...
                                        {{-- Ban user --}}
                                        @if ($r->reported->status !== 'banned')
                                            @can('ban_user')
                                            <button wire:key="option-ban-{{ $r->id }}" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_ban_this_user') }}') ? $wire.ban('{{ $r->id }}') : ''" wire:loading.attr="disabled" wire:target="ban('{{ $r->id }}')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >

                                                {{-- Loading indicator --}}
                                                <div wire:loading wire:target="ban('{{ $r->id }}')">
                                                ...
                                                {{-- Icon --}}
                                                <div wire:loading.remove wire:target="ban('{{ $r->id }}')">
