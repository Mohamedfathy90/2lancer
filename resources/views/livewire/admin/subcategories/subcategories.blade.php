        ...
        <table class="w-full whitespace-nowrap old-tables">
            <thead class="bg-gray-200">
                <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800 dark:text-white">
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4">{{ __('messages.t_subcategory') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4">{{ __('messages.t_parent_category') }}</th>
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">{{ __('messages.t_social_media_image') }}</th>
            ...
            <tbody class="w-full">
                ...
                @foreach ($subcategories as $c)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40" wire:key="subcategories-{{ $c->id }}">

                        {{-- Subcategory --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('categories/' . $c->parent->slug . "/" . $c->slug) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($c->icon) }}" alt="{{ $c->name }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="font-medium text-sm">{{ $c->name }}</p>
                                    <p class="text-xs leading-3 text-gray-600 pt-2 max-w-xs truncate block">{{ $c->description }}</p>
                                </div>
                            </a>
                        </td>
                        ...
                        {{-- Parent category --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('categories', $c->parent->slug) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($c->parent->icon) }}" alt="{{ $c->parent->name }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="font-medium text-sm">{{ $c->parent->name }}</p>
                                    <p class="text-xs leading-3 text-gray-600 pt-2 max-w-xs truncate block">{{ $c->parent->description }}</p>
                                </div>
                            </a>
                        </td>
                        ...
                        <td class="text-center">
                            <img class="w-8 h-8 rounded object-cover mx-auto lazy" src="{{ placeholder_img() }}" data-src="{{ src($c->image) }}" alt="{{ $c->name }}" />
                        </td>
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div id="table-options-dropdown-{{ $c->id }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">
                                        ...
                                        <a href="{{ admin_url('subcategories/edit/' . $c->uid) }}" class="text-gray-800 dark:text-gray-300 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                            ...
                                            <span class="text-xs font-medium">{{ __('messages.t_edit_subcategory') }}</span>
                                        </a>
                                        @endcan

                                        {{-- Delete subcategory --}}
                                        @can('delete_subcategory')
                                        <button x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $c->id }}') : ''" wire:loading.attr="disabled" wire:target="delete('{{ $c->id }}')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                            ...
                                            <span class="text-xs font-medium">{{ __('messages.t_delete_subcategory') }}</span>
