    ...
    <div class="px-4 md:px-3 py-4 md:py-5 bg-white border !border-b-0 dark:bg-gray-700 rounded-tl-lg rounded-tr-lg">
        <div class="sm:flex items-center justify-between">
            <p class="text-sm font-bold leading-wide text-gray-800">
                {{ __('messages.t_categories') }}
            </p>
            @can('add_category')
            <div>
                <a href="{{ admin_url('categories/create') }}" class="focus:ring-2 focus:ring-offset-2 focus:ring-primary-600 inline-flex sm:ml-3 mt-4 sm:mt-0 items-start justify-start px-6 py-3 bg-primary-600 hover:bg-primary-700 focus:outline-none rounded-sm">
        ...
        <table class="w-full whitespace-nowrap old-tables">
            <thead class="bg-gray-200">
                <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800 dark:text-white">
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4">{{ __('messages.t_category') }}</th>
                    ...
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">{{ __('messages.t_visibility') }}</th>
            ...
            <tbody class="w-full">
                ...
                @foreach ($categories as $c)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40" wire:key="categories-{{ $c->id }}">

                        {{-- Category --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('categories', $c->slug) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div id="table-options-dropdown-{{ $c->id }}" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">
                                        ...
                                        {{-- Edit category --}}
                                        @can('edit_category')
                                        <a href="{{ admin_url('categories/edit/' . $c->uid) }}" class="text-gray-800 dark:text-gray-300 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                            ...
                                            <span class="text-xs font-medium">{{ __('messages.t_edit_category') }}</span>
                                        </a>
                                        @endcan
                                        ...
                                        <button x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $c->id }}') : ''" wire:loading.attr="disabled" wire:target="delete('{{ $c->id }}')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                            ...
                                            <span class="text-xs font-medium">{{ __('messages.t_delete_category') }}</span>
