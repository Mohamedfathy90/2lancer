...
{    
    ...
    public function mount()
    {
        try {
            ...
            if ( $payment ) {
                ...
                // Get paid amount
                $amount   = $payment->getBaseAmount() ?? $payment->getAmount();

                // Get currency
                $currency = $payment->getCurrency() ?? $payment->getBaseCurrency();

                // Check currency
                if (strtolower($currency) != strtolower($settings->currency)) {
    ...
    public function checkout($payment_id)
    {
        try {
            ...
            $invoice->payment_method = 'youcanpay';
            ...
            auth()->user()->notify( (new OrderPlaced($order))->locale(config('app.locale')) );
