...
class RegisterComponent extends Component
{
    ...
    public function mount()
    {
        ...
        // check for affiliate
        if(Request()->has('ref') && settings('affiliate')->is_enabled){
        
        // add affiliate click 
        AffiliateClick::create([
            'referral_id' => Request('ref')
        ]);    
        
        $this->ref_id = Request('ref');
    ...
    public function register($form)
    {
        ...
        try {
            ...
            // Verify form first
            if (!is_array($form) || !isset($form['email']) || !isset($form['password']) || !isset($form['phone']) || !isset($form['fullname']) || !isset($form['username'])) {
                return;
            }
            ...
            $user->phone    = $this->countryCode.clean($this->phone);
            ...
            if($this->ref_id && settings('affiliate')->is_enabled){
            $expiry_months = settings('affiliate')->expiry_months;
            $affiliate_register = new AffiliateRegisteration();
            $affiliate_register->user_id = $user->id ;
            $affiliate_register->referral_id = $this->ref_id ;
            $affiliate_register->expires_at = Carbon::now()->addMonths($expiry_months); 
            $affiliate_register->save();
            }
