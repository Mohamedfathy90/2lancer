    ...
    public function getGigsProperty()
    {
        ...
        if ($this->sort_by) {
            switch ($this->sort_by) {
                ...
                case 'rand':
                    $rated_gigs  = Gig::where('subcategory_id', $this->subcategory->id)->where('counter_reviews','>',0)->active()->orderByDesc('rating')->get(); 
                    $random_gigs = Gig::where('subcategory_id', $this->subcategory->id)->where('counter_reviews',0)->active()->orderByRaw('RAND()')->get();                  
                    $result_gigs = [];
                    ...
                    $query = $query->whereIn('id', $result_gigs)->orderByRaw("FIELD(id, $order)");     
                    break;
            }
        }
