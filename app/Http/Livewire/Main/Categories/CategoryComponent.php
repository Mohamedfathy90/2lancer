    ...
    public function getGigsProperty()
    {
        ...
        if ($this->sort_by) {
            switch ($this->sort_by) {
                ...
                case 'rand':
                    $rated_gigs  = Gig::where('category_id', $this->category->id)->where('counter_reviews','>',0)->active()->orderByDesc('rating')->get(); 
                    $random_gigs = Gig::where('category_id', $this->category->id)->where('counter_reviews',0)->active()->orderByRaw('RAND()')->get();                  
                    $result_gigs = [];
