...
class HomeComponent extends Component
{
    ...
    public $affiliate_profits;
    public $affiliate_total_income;
    ...
    public function mount()
    {
        ...
        // Calculate Affiliate Users
        $this->affiliate_users = AffiliateRegisteration::count();
        
        // Calculate Affiliate Orders
        $this->affiliate_orders = AffiliateTransaction::count();

        // Calculate Affiliate Profits
        $this->affiliate_profits = AffiliateTransaction::sum('referral_earning');

        // Calculate Affiliate total income
        $this->affiliate_total_income = 0 ;
        $affiliateTransactions = AffiliateTransaction::all();
        if($affiliateTransactions){
        foreach($affiliateTransactions as $transaction){
            $this->affiliate_total_income += $transaction->order->total_value ; 
        }
        }
