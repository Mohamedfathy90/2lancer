...
class FreekassaController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        try {
            ...
            $allowed_ip     = ['168.119.157.136', '168.119.60.227', '138.201.88.124', '178.154.197.79'];

            // Check webhook event
            if ( in_array($ip_address, $allowed_ip) ) {

                // Set merchant id
                $merchant_id     = $settings?->settings['shop_id'];

                // Set merchant secret
                $merchant_secret = $settings?->settings['merchant_password2'];
    ...
    public function failed(Request $request)
    {
        try {
            ...
            // Check if deposit callback
            if (Str::startsWith($merchant_order_id, 77)) {
                        
                // Get saved webhook data in our database
                DepositWebhook::where('payment_id', $merchant_order_id)
                                ->where('payment_method', $this->gateway)
                                ->where('status', 'pending')
                                ->delete();
            ...
            // Check if checkout callback
            if (Str::startsWith($merchant_order_id, 88)) {
                
                // Get saved webhook data in our database
                CheckoutWebhook::where('payment_id', $merchant_order_id)
                                ->where('payment_method', $this->gateway)
                                ->where('status', 'pending')
                                ->delete();
    ...
    public function success(Request $request)
    {
        try {
            ...
            if ( $order_id ) {
                ...
                } else if (Str::startsWith($order_id, 88)) {
