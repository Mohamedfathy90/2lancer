...
class DuitkuController extends Controller
{
    ...
    private function verify($id)
    {
        try {
            ...
            $merchantCode    = $this->settings?->settings['merchant_code'];
            $apiKey          = $this->settings?->settings['api_key'];

            // Generate signature
            $signature       = md5($merchantCode . $id . $apiKey);
            ...
            // Check transaction
            $response = Http::post($request_url, $params)->json();
