...
class PaystackController extends Controller
{
    ...
    public function callback(Request $request)
    {
        ...
        } else if (Str::startsWith($order_id, "GG")) {
    ...
    public function webhook(Request $request)
    {
        try {
            ...
            // Check webhook event
            if ( isset($fields['event']) && $fields['event'] === 'charge.success' && $fields['data']['status'] === 'success' ) {
                
                // Get order id
                $order_id = $fields['data']['reference'];
    ...
    private function verify($id)
    {
        try {
            ...
            // Check if payment succeeded
            if ( isset($response['status']) && isset($response['data']['status']) && $response['data']['status'] === 'success' ) {
