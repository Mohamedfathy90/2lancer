...
class JazzcashController extends Controller
{
    ...
    public function callback(Request $request)
    {
        try {
            ...
            if ( $transaction_id ) {
                ...
                $response = $this->verify($transaction_id, $request->get('token'));
                
                // Check if payment succeeded
                if ( is_array($response) && $response['success'] === TRUE) {

                    // Check if deposit callback
                    if (Str::startsWith($transaction_id, "DD")) {
                    ...
                    if (Str::startsWith($transaction_id, "GG")) {
     ...
     * @return array
     */
    private function verify($id, $token)
    {
        try {

            // Set payment gateway settings
            $env              = $this->settings?->settings['env'];
            $endpoint         = $env === 'sandbox' ? $this->settings?->settings['sandbox_link'] : $this->settings?->settings['production_link'] ;
            $merchant_id      = $this->settings?->settings['merchant_id'];
            $password         = $this->settings?->settings['password'];
            $salt             = $this->settings?->settings['integerity_salt'];
            ...
            $pp_secure_hash   = generate_jazzcash_hash($order);
            ...
            // Check if payment succeeded
            if ( is_array($response) && isset($response['pp_ResponseCode']) && $response['pp_ResponseCode'] == "000" ) {
