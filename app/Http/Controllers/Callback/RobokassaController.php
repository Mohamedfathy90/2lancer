...
class RobokassaController extends Controller
{
    ...
    private function verify($request)
    {
        try {
            ...
            // Get payment gateway settings
            $mrh_login = $this->settings?->settings['mrh_login'];
            $mrh_pass1 = $this->settings?->settings['mrh_pass1'];
            $mrh_pass2 = $this->settings?->settings['mrh_pass2'];

            // Get payment details
            $payment   = new \Lexty\Robokassa\Payment(
                new \Lexty\Robokassa\Auth($mrh_login, $mrh_pass1, $mrh_pass2, true)
            );
