...
class PaytrController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        try {
            ...
            // Check webhook event
            if ( isset($fields['hash']) && isset($fields['merchant_oid']) && isset($fields['status']) ) {

                // Check if payment succeeded
                $response = $this->verify($fields);
    ...
    private function verify($request)
    {
        try {
            ...
            $merchant_salt = $this->settings?->settings['merchant_salt'];
            $merchant_key  = $this->settings?->settings['merchant_key'];

            // Get paytr response secure hash
            $paytr_hash    = $request['hash'];

            // Get order id
            $order_id      = $request['merchant_oid'];
            ...
            $secure_hash   = base64_encode(hash_hmac( 'sha256', $order_id . $merchant_salt . $status . $total_amount, $merchant_key, true ));
