...
class CpayController extends Controller
{
    ...
    public function failed(Request $request)
    {
        try {
            ...
            if (Str::startsWith($external_reference, "DD")) {
                        
                // Get saved webhook data in our database
                DepositWebhook::where('payment_id', $external_reference)
            ...
            if (Str::startsWith($external_reference, "GG")) {
    ...
    private function verify($id)
    {
        try {
            ...
            $username     = $gateway?->settings['app_username'];
            $password     = $gateway?->settings['app_password'];
            $request_link = $gateway?->settings['env'] === 'sandbox' ? $gateway?->settings['sandbox_link'] : $gateway?->settings['production_link'];

            // Send request
            $token        = Http::post("$request_link/api/token/", [
                                    "username" => $username,
                                    "password" => $password
                                ])->json();
            ...
            if ( isset($token['token']) ) {
                ...
                $response = Http::withToken($token['token'], 'Token')->get("$request_link/api/transaction/$id")->json();

                // Check if payment succeeded
                if ( is_array($response) && $response['status'] === 'SUCCESSFUL' ) {
