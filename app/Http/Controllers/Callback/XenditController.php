...
class XenditController extends Controller
{
    ...
    public function success(Request $request)
    {
        try {
            ...
            if ($action === "D") {
                ...
                // Redirecting
                return $this->redirect('deposit', 'success');
            ...
            if ($action === "G") {
                ...
                // Redirecting
                return $this->redirect('gigs', 'success');
    ...
    public function webhook(Request $request)
    {
        try {
            ...
            // Get order id
            $order_id       = $request->get('external_id');

            // Check webhook event
            if ( $invoice_id && $order_id ) {

                // Check if payment succeeded
                $response = $this->verify($invoice_id);
    ...
    private function verify($id)
    {
        try {
            ...
            // Get invoice
            $invoice    = \Xendit\Invoice::retrieve($id);

            // Check if payment succeeded
            if ( is_array($invoice) && in_array($invoice['status'], ['PAID', 'SETTLED']) ) {
