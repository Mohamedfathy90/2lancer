...
class MercadopagoController extends Controller
{
    ...
    public function failed(Request $request)
    {
        try {
            ...
            if (Str::startsWith($payment_id, "DD")) {
                        
                // Get saved webhook data in our database
                DepositWebhook::where('payment_id', $payment_id)
            ...
            if (Str::startsWith($payment_id, "GG")) {
                
                // Get saved webhook data in our database
                CheckoutWebhook::where('payment_id', $payment_id)
                                ->where('payment_method', $this->gateway)
                                ->where('status', 'pending')
                                ->delete();
            ...
            if ( $payment_id ) {
                ...
                if ( is_array($response) && $response['success'] === TRUE) {
                    ...
                    if (Str::startsWith($payment_id, "DD")) {
                        ...
                        // Get saved webhook data in our database
                        $data = DepositWebhook::where('payment_id', $payment_id)
                                                ->where('payment_method', $this->gateway)
                                                ->where('status', 'pending')
                                                ->firstOrFail();
        
                        // Handle deposit callback
                        $this->deposit($data->user_id, $data->amount, $payment_id);
                    ...
                    if (Str::startsWith($payment_id, "GG")) {
                        ...
                        // Get saved webhook data in our database
                        $data = CheckoutWebhook::where('payment_id', $payment_id)
                                                ->where('payment_method', $this->gateway)
                                                ->where('status', 'pending')
                                                ->firstOrFail();
                        ...
                        // Handle deposit callback
                        $this->checkout($cart, $user, $payment_id);
    ...
    private function verify($id)
    {
        try {
            ...
            \MercadoPago\SDK::setAccessToken( $this->settings?->settings['access_token'] );
