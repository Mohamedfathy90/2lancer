...
class ChatApi
{
    ...
    public function newMessage($data)
    {
        ...
        $message->save();
        $chat_messages = Message::where('from_id',$data['from_id'])->where('to_id',$data['to_id'])->get()->count();
        $to_user = User::find($data['to_id']);
        
        try{
        if($chat_messages == 1 && $to_user->phone){
            $account_sid = getenv("TWILIO_ACCOUNT_SID");
            $auth_token = getenv("TWILIO_AUTH_TOKEN");
            $twilio_service_sid = getenv("TWILIO_SERVICE_SID");
            $twilioWhatsAppNumber = getenv("TWILIO_WHATSAPP_NUMBER");
            $template_sid = "HX2c87078f55fb3141837ba97c2297d1e6";
            $recipientNumber = "whatsapp:+".$to_user->phone;
