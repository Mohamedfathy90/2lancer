    ...
    <div class="bg-white dark:bg-zinc-800 overflow-y-auto border !border-t-0 !border-b-0 dark:border-zinc-600">
            ...
            <div class="ltr:ml-2 rtl:mr-2 w-full hidden lg:block mb-4">   
                <div class="relative max-w-md" x-data="{ open: false }">
                    ...
                    <input wire:model.debounce.500ms="q" wire:keydown.enter="enter" x-ref="search" x-on:click="open = true" type="search" class="block p-2.5 w-full z-20 text-sm text-gray-900 dark:text-white bg-white dark:bg-[#181818] rounded border border-gray-300 dark:border-[#181818] focus:ring-0 focus:border-gray-500 h-10" placeholder="<?php echo e(__('messages.t_search_for_user'), false); ?>" required>
                                ...
                                <div id="table-options-dropdown-<?php echo e($user->id, false); ?>" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">
                                        ...
                                        <a href="<?php echo e(admin_url('users/edit/' . $user->uid), false); ?>" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1">
                                            ...
                                            <span class="text-xs font-medium"><?php echo e(__('messages.t_edit_user'), false); ?></span>
                                        </a>
                                        <?php endif; ?>
                                        ...
                                        <a href="<?php echo e(admin_url('users/details/' . $user->uid), false); ?>" class="text-gray-800 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1">
                                            ...
                                            <span class="text-xs font-medium"><?php echo e(__('messages.t_user_details'), false); ?></span>
                                        </a>
                                        <?php endif; ?>
                                        ...
                                        <?php if($user->status === 'pending'): ?>
                                            ...
                                            <button wire:key="option-activate-<?php echo e($user->id, false); ?>" wire:click="activate('<?php echo e($user->id, false); ?>')" wire:loading.attr="disabled" wire:target="activate('<?php echo e($user->id, false); ?>')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                                ...
                                                <span class="text-xs font-medium"><?php echo e(__('messages.t_activate_user'), false); ?></span>
                                        ...
                                        <?php if($user->status !== 'banned'): ?>
                                           <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('ban_user')): ?> 
                                            <button wire:key="option-ban-<?php echo e($user->id, false); ?>" x-on:click="confirm('<?php echo e(__('messages.t_are_u_sure_u_want_to_ban_this_user'), false); ?>') ? $wire.ban('<?php echo e($user->id, false); ?>') : ''" wire:loading.attr="disabled" wire:target="ban('<?php echo e($user->id, false); ?>')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
                                        ...
                                        <button wire:key="option-delete-<?php echo e($user->id, false); ?>" x-on:click="confirm('<?php echo e(__('messages.t_are_u_sure_u_want_to_delete_this_user'), false); ?>') ? $wire.delete('<?php echo e($user->id, false); ?>') : ''" wire:loading.attr="disabled" wire:target="delete('<?php echo e($user->id, false); ?>')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >

                                            
                                            <div wire:loading wire:target="delete('<?php echo e($user->id, false); ?>')">
                                                <svg role="status" class="ltr:mr-3 rtl:ml-3 inline w-5 h-5 text-gray-500 animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB"/>
                                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor"/>
                                                </svg>
                                            ...
                                            <div wire:loading.remove wire:target="delete('<?php echo e($user->id, false); ?>')">
                                            ...
                                            <span class="text-xs font-medium"><?php echo e(__('messages.t_move_to_trash'), false); ?></span>
