            ...
            <div class="col-span-12 mt-6 xl:mt-6 mb-16">
                ...
                <div class="flex-wrap justify-center items-center mt-8 -mx-5 hidden" id="featured-categories-slick" wire:ignore>
                    ...
                    <a href="<?php echo e(url('categories', $category->slug), false); ?>" class="relative !h-72 rounded-lg !p-6 !flex !flex-col overflow-hidden group mx-5">
                        ...
                        <span class="relative mt-auto text-center text-xl font-bold text-white"><?php echo app('translator')->get('messages.t_'. $category->name); ?></span>
                    </a>
            ...
            <?php if($category->gigs()->active()->count()): ?>
                ...
                <div class="col-span-12">
                    <div class="flex justify-between items-center bg-transparent py-2">
                        ...
                        <div>
                            <span class="font-extrabold text-xl text-gray-800 dark:text-gray-100 pb-1 block tracking-wider"><?php echo app('translator')->get('messages.t_'.$category->name); ?></span>
                        </div>
                ...
                <div class="col-span-12 mb-16">
                    <div class="grid grid-cols-12 sm:gap-x-9 gap-y-6">
                        <?php
                        $rated_gigs  = \App\Models\Gig::where('category_id', $category->id)->where('counter_reviews','>',0)->active()->orderByDesc('rating')->get(); 
                        $random_gigs = \App\Models\Gig::where('category_id', $category->id)->where('counter_reviews',0)->active()->inRandomOrder()->get();                  
                        $result_gigs = [];
                        
                        foreach($rated_gigs as $gig){
                            $result_gigs[]= $gig->id ;
                        }
                        foreach($random_gigs as $gig){
                            $result_gigs[]= $gig->id ;
                        }

                        $order = implode(',', $result_gigs); 
                        $category_gigs = \App\Models\Gig::whereIn('id', $result_gigs)->orderByRaw("FIELD(id, $order)")->take(4)->get();
                        ?>                    
                        
                        <?php $__currentLoopData = $category_gigs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gig): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\xampp\htdocs\2lancer\resources\views/livewire/main/home/home.blade.php ENDPATH**/ ?>