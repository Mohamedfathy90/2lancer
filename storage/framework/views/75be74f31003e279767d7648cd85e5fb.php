                            ...
                            <div>
                                <div>
                                    <div class="md:flex md:items-center md:justify-between md:space-x-4 md:rtl:space-x-reverse xl:border-b xl:pb-6 dark:xl:border-zinc-700">
                                        <div>
                                            <h1 class="text-base font-bold text-gray-900 dark:text-gray-100"><?php echo e(__('messages.t_refund_details'), false); ?></h1>
                                            <div class="mt-2 text-xs font-medium tracking-wide text-primary-600 hover:text-primary-600 flex items-center">
                                                ...
                                                <a href="<?php echo e(url('service', $refund->item->gig->slug), false); ?>" target="_blank" class="ltr:ml-2 rtl:mr-2"><?php echo e($refund->item->gig->title, false); ?></a>
                                            </div>
                                        </div>
                                        <div class="mt-4 flex md:mt-0">
                                            <?php if($refund->status === 'rejected_by_seller' && !$refund->request_admin_intervention): ?>
                                                <button x-on:click="confirm('<?php echo e(__('messages.t_are_u_sure_raise_dispute_refund', ['app_name' => config('app.name')]), false); ?>') ? $wire.raise : ''" wire:loading.attr="disabled" wire:target="raise" type="button" class="inline-flex justify-center items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900">
                                                    ...
                                                    <span class="text-xs font-medium"><?php echo e(__('messages.t_raise_a_dispute'), false); ?></span>
                                                </button>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    ...
                                    <aside class="mt-8 xl:hidden">
                                        <div class="space-y-5">
                                            ...
                                            <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                                ...
                                                <span class="text-gray-700 dark:text-gray-300 text-sm font-black tracking-wide "><?php echo money($refund->item->total_value, settings('currency')->code, true); ?></span>
                                            </div>
                                            ...
                                            <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                                ...
                                                <span class="text-gray-700 dark:text-gray-300 text-sm font-medium"><?php echo e(__('messages.t_created_on_date', ['date' => format_date($refund->created_at, config('carbon-formats.F_j,_Y'))]), false); ?></span>
                                            </div>
                                        ...
                                        <?php if($refund->status === 'rejected_by_seller' && $refund->request_admin_intervention): ?>
                                            <div class="flex items-center">
                                                ...
                                                <p class="ltr:ml-2 rtl:mr-2 text-xs font-medium text-amber-600">
                                                    <?php echo e(__('messages.t_our_team_investigate_refund_request_now'), false); ?>
                                    ...
                                    <div class="divide-y divide-gray-200 dark:divide-zinc-700">
                                        <div class="pb-4">
                                            <h2 class="text-base font-extrabold tracking-wider text-gray-900 dark:text-gray-100">
                                                <?php echo e(__('messages.t_conversation'), false); ?>
                                                ...
                                                <div class="mt-6">
                                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                                        <div class="flex-shrink-0">
                                                            <div class="relative">
                                                                <img class="lazy h-10 w-10 rounded-full bg-gray-400 dark:bg-zinc-800 flex items-center justify-center ring-8 ring-white dark:ring-transparent object-cover" src="<?php echo e(placeholder_img(), false); ?>" data-src="<?php echo e(src(auth()->user()->avatar), false); ?>" alt="<?php echo e(auth()->user()->username, false); ?>">
                                                            </div>
                                                        </div>
                                                        <div class="min-w-0 flex-1">
                                                            <div>
                                                                <textarea wire:model.defer="message" rows="4" class="shadow-sm block w-full dark:bg-transparent focus:ring-gray-900 dark:focus:ring-primary-600 focus:border-gray-900 dark:focus:border-primary-600 sm:text-sm border border-gray-300 dark:border-zinc-700 dark:text-gray-200 rounded-md resize-none" placeholder="<?php echo e(__('messages.t_enter_your_message'), false); ?>" maxlength="500"></textarea>
                                                            </div>
                                                            <div class="mt-6 flex items-center justify-end space-x-4 rtl:space-x-reverse">
                                                                <button x-on:click="confirm('<?php echo e(__('messages.t_are_u_sure_close_refund_dispute'), false); ?>') ? $wire.close : ''" wire:loading.attr="disabled" wire:target="close" type="button" class="inline-flex justify-center px-4 py-2 border border-gray-300 dark:border-zinc-700 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 h-12 items-center">
                                                                    ...
                                                                    <span class="text-xs font-medium"><?php echo e(__('messages.t_close_refund'), false); ?></span>
                                                                </button>
...
<?php endif; ?>
                        ...
                        <aside class="hidden xl:block xl:ltr:pl-8 xl:rtl:pr-8">
                            <div class="space-y-5">
                                ...
                                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor"> <path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4z"/> <path fill-rule="evenodd" d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" clip-rule="evenodd"/></svg>
                                    <span class="text-gray-700 dark:text-gray-200 text-sm font-black tracking-wide "><?php echo money($refund->item->total_value, settings('currency')->code, true); ?></span>
                                </div>

                                
                                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor"> <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/></svg>
                                    <span class="text-gray-700 dark:text-gray-200 text-sm font-medium"><?php echo e(__('messages.t_created_on_date', ['date' => format_date($refund->created_at, config('carbon-formats.F_j,_Y'))]), false); ?></span>
                                </div>
...
<?php /**PATH /home/u100719507/domains/2lancer.ma/public_html/resources/views/livewire/main/account/refunds/options/details.blade.php ENDPATH**/ ?>