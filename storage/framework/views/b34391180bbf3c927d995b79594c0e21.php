                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 px-8 py-6 mb-8">
                    ...
                    <div class="mb-14">
                        <h2 class="text-base tracking-wide font-bold text-zinc-900 dark:text-gray-100">
                            <?php echo e(__('messages.t_deliver_completed_work_subtitle'), false); ?>
                    ...
                    <?php if($order->delivered_work): ?>
                                ...
                                <div class="min-w-0 flex-1 flex items-center space-x-3 rtl:space-x-reverse">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-200 truncate pb-2">
                                            <?php echo e($order->delivered_work->attached_work['id'], false); ?>.<?php echo e($order->delivered_work->attached_work['extension'], false); ?>

                                        </p>
                                        <p class="text-xs font-[400] text-gray-400 truncate">
                                            <?php echo e(human_filesize($order->delivered_work->attached_work['size']), false); ?>

                                        </p>
                                    </div>
                                </div>
                                <div class="flex-shrink-0">
                                    <a href="<?php echo e(url('uploads/delivered/' . $order->order->uid . '/' . $order->uid . '/' . $order->delivered_work->id . '/' . $order->delivered_work->attached_work['id']), false); ?>" target="_blank" class="inline-flex items-center py-2 px-4 border border-transparent rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 focus:outline-none focus:ring-0">
                        ...
                        <?php if($order->delivered_work->quick_response): ?>
                            ...
                            <div class="flex flex-col space-y-4 overflow-y-auto mb-6">
                                <div class="flex items-center">
                                    ...
                                    <img src="<?php echo e(placeholder_img(), false); ?>" data-src="<?php echo e(src(auth()->user()->avatar), false); ?>" alt="<?php echo e(auth()->user()->username, false); ?>" class="lazy w-10 h-10 rounded-full order-1 object-cover">
                                </div>
                            </div>
                        ...
                        <?php if(!$order->is_finished): ?>
                            <div class="mt-12 mb-4">
                                <button x-on:click="confirm('<?php echo e(__("messages.t_are_u_sure_u_want_to_resubmit_work_again"), false); ?>') ? $wire.resubmit() : ''" type="button" class="text-white w-full bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-0 font-medium rounded-md text-sm px-5 py-4 text-center inline-flex items-center justify-center" wire:loading.attr="disabled" wire:target="resubmit">
                                    ...
                                    <div wire:loading.remove wire:target="resubmit">
                                        <?php echo e(__('messages.t_resubmit_work_again'), false); ?>
                    ...
                    <?php elseif(!$order->is_finished): ?>
                        <div class="grid grid-cols-12 md:gap-x-6 gap-y-10">
                            ...
                            <div class="col-span-12" wire:ignore>
                                ...
                                <span class="mb-2 text-[0.8125rem] font-semibold tracking-wide flex items-center text-gray-700 dark:text-white"><?php echo e(__('messages.t_upload_work'), false); ?></span>
...
<?php $attributes = $attributes->except(collect($constructor->getParameters())->map->getName()->all()); ?>
<?php endif; ?>
<?php $component->withAttributes(['model' => 'work','id' => 'uploader_work','extensions' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(['zip']),'accept' => 'application/zip','size' => ''.e(settings('media')->delivered_work_max_size, false).'','max' => '1']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
                                ...
                                <span class="text-xs text-gray-400 font-normal">
                                    <?php echo e(__('messages.t_only_zip_allowed_max_size', ['size' => settings('media')->delivered_work_max_size]), false); ?>
                            ...
                            <div class="mx-auto">
                                <div class="py-3 px-1 md:flex md:items-center md:justify-between lg:border-b lg:border-gray-200 lg:dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800 rounded-t-lg">
                                    <div class="flex-1 min-w-0 px-3">
                                        <div class="flex items-center">
                                            <img class="hidden h-10 w-10 rounded-full sm:block object-cover lazy" src="<?php echo e(placeholder_img(), false); ?>" data-src="<?php echo e(src($order->order->buyer->avatar), false); ?>" alt="<?php echo e($order->order->buyer->username, false); ?>">
                                            <div>
                                                <div class="flex items-center">
                            ...
                            <?php $__currentLoopData = $order->conversation; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li wire:key="seller-deliver-order-msg-id-<?php echo e($message->id, false); ?>">
                                    ...
                                    <div class="relative flex items-start space-x-3 rtl:space-x-reverse">
                                        <div class="relative">
                                            <img class="h-10 w-10 rounded-full bg-gray-400 flex items-center justify-center ring-8 ring-white dark:ring-transparent object-cover lazy" src="<?php echo e(placeholder_img(), false); ?>" data-src="<?php echo e(src($message->from->avatar), false); ?>" alt="<?php echo e($message->from->username, false); ?>">
                                        </div>
                                        <div class="min-w-0 flex-1">
                                        <div>
                                            <div class="text-sm">
                                                <a href="<?php echo e(url('profile', $message->from->username), false); ?>" target="_blank" class="font-bold tracking-wide text-gray-900 dark:text-gray-100"><?php echo e($message->from->username, false); ?></a>
                                            </div>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400"><?php echo e(format_date($message->created_at, 'ago'), false); ?></p>
                                        </div>
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="relative">
                                        <div class="border border-gray-300 dark:border-zinc-600 rounded-lg shadow-sm overflow-hidden focus-within:border-primary-600 focus-within:ring-1 focus-within:ring-primary-600">
                                            <textarea rows="3" maxlength="750" wire:model.defer="message" class="block w-full py-3 border-0 resize-none focus:ring-0 sm:text-sm dark:bg-transparent dark:text-gray-200" placeholder="<?php echo e(__('messages.t_type_ur_message_here'), false); ?>"></textarea>
                                        ...
                                        <div class="absolute bottom-0 inset-x-0 ltr:pl-3 rtl:pr-3 ltr:pr-2 rtl:pl-2 py-2 flex justify-between">
                                            <div></div>
                                            <div class="flex-shrink-0">
                                                <button wire:click="sendMessage" wire:loading.attr="disabled" wire:target="sendMessage" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-600"><?php echo e(__('messages.t_send'), false); ?></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
</div><?php /**PATH /home/u100719507/domains/2lancer.ma/public_html/resources/views/livewire/main/seller/orders/options/deliver.blade.php ENDPATH**/ ?>